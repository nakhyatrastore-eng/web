      ...
      <div className="page-shell grid gap-8 py-10 lg:grid-cols-[1.05fr_.95fr] lg:py-16">
        <section className="lg:sticky lg:top-24 lg:self-start">
          <div className={`relative mx-auto overflow-hidden border border-accent/60 bg-surface shadow-[14px_14px_0_rgb(124_58_237_/_0.7)] ${phoneCase ? 'aspect-[9/18] max-w-[390px] rounded-[36px]' : 'aspect-[4/5] max-w-[620px]'}`}>
            ...
            {phoneCase ? <img src="/images/iphone-11-pro-gloss-template.png" alt="" aria-hidden="true" className="pointer-events-none absolute left-1/2 top-1/2 z-10 max-w-none w-[238%] -translate-x-1/2 -translate-y-1/2" /> : <div className="pointer-events-none absolute inset-3 border border-white/20" />}
          </div>
        ...
        <section className="border border-line bg-bg p-5 md:p-7">
          ...
          {product.variants.length > 1 ? <ProductOptions variants={product.variants} selectedId={variant?.id ?? null} onChange={(item) => setVariantIds((current) => ({ ...current, [product.id]: item?.id ?? '' }))} /> : null}
